...
pub fn parse(input: Vec<Token>) -> Result<AST, &'static str> {
	let mut ast = AST::Leaf;		// this would be a good place to find entry point
									// ast has to be initialized
	let mut input = input.clone();

	while !input.is_empty() {
		if is_statement_start(&input[0]) {
			let (node, rem) = try_parse_stmt(input)?;
			input = rem;
			ast = AST::StmtNode(node, Box::new(ast));
		} else {
			let (node, rem) = try_parse_exp(input)?;
			input = rem;
			ast = AST::ExpNode(node, Box::new(ast));
		}
	}
...
type ParseResult<T> = Result<(T, Vec<Token>), &'static str>;

fn try_parse_stmt(input: Vec<Token>) -> ParseResult<Statement> {
...
fn is_statement_start(tk: &Token) -> bool {
	return match tk {
		Token::Let => true,
		Token::Function => true,
		Token::Return => true,
